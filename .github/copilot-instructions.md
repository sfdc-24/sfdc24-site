...
`.github/copilot-instructions.md` covering the fleet's conventions. Instructions
do not propagate between the two repos, and the two have different risk profiles —
Blackboard is source and tooling, this one is production.
...
| Public routes | `/`, `/voice/`, `/governor/`, `/intake/`, `/projects/`, `/privacy/`, `/terms/`, plus `404.html` |
...
- **Unlisted is a real state.** Some routes are deliberately absent from
  `sitemap.xml` — reachable but not advertised. Do not "fix" a missing sitemap
  entry without checking whether it is deliberate.
...
- Tests: Python `unittest` under `tests/`, run by a path-scoped workflow. Add a
  new workflow for a new area rather than widening an existing job's discovery
  pattern, so a reviewed job keeps running exactly what was reviewed.
