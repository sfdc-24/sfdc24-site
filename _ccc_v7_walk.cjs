...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector("[data-cd-tally]");
    rows.push({ stage: "tally", cards: await page.locator("#choice-design-probe .cd-profile").count(),
      S9: await page.locator(S9).count(), OLD: await page.locator(OLD).count(), rendered: "ok" });
    await page.locator("[data-cd-again]").click();
    await page.waitForSelector("#choice-design-probe .cd-profile");
    rows.push({ stage: "after-Again", cards: await page.locator("#choice-design-probe .cd-profile").count(),
      S9: await page.locator(S9).count(), OLD: await page.locator(OLD).count(),
      rendered: ((await page.locator("[data-cd-status]").first().textContent()) || "").includes("Set 1 of 8") ? "ok" : "MISMATCH" });
