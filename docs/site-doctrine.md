...
**Ownership:** unassigned since 2026-09-24, when Foundry (which owned the Daily SPEED TEST and its WhatsApp results) was dropped from the fleet and Grok (the staging→production speed gate) was reserved; the feed's last measured row is 2026-09-18. Visitor copy must not name an owner until one runs it. Copilot owns staging. Charts live under **Method** at `/method/#speed` (alias `/speed/` redirects there). Shared chrome only — no new layout language. Feed series from `data/speed-test-log.jsonl` and board receipts. Mark example data clearly until real points exist. Lightweight SVG/canvas only; no heavy chart libraries.
...
- **Choice-design Method probe:** `/method/#choice-design` walks the catalog attributes and levels as an on-page preference probe. Catalog status stays `pilot-not-deployed` until a real experiment and n≥20+CI. Cobalt stays locked (`#0A66C2`). No cookies. Does not switch the ask bar or Release rail.
