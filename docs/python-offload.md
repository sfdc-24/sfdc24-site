...
| Ops snap for `/ops/` | `tools/board_ops_snap.py` writes `data/board-ops-snap.json`. The page polls that static file. The scheduled workflow commits to branch `board-ops-snap`, never `main`. No bus call. `/operating-model/` redirects to `/ops/`. | Whether a baked failure is real, and when a sanitized export should replace the quiet roster |
