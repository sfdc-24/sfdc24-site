...
def extract_strings(html: str) -> list[dict]:
    """Lightweight inventory of visible-ish string literals and key CSS rules."""
    items: list[dict] = []
    # quoted JS/CSS string literals >= 12 chars
    for m in re.finditer(r'(["\'])([^"\']{12,200})\1', html):
        s = m.group(2)
        if s.startswith("http") or "function " in s:
            continue
        items.append({
            "text": s,
            "offset": m.start(),
