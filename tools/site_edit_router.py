...
Input JSON:
  {
    ...
    "target": "index.html",          # must be in manifest paths (homepage scope)
    "kind": "find_replace" | "css_var" | "static_section" | "triage_answer",
    "find": "...",                   # find_replace
    "replace": "...",
    "var": "--soft", "value": "#333",  # css_var
    "section_id": "...", "section_cls": "...", "section_body": "...",
    "triage_id": "...", "answer": "...",
    "dry_run": true,
    "branch": "optional-branch-name"
  }

Cheap path: apply patch, optional git branch+commit.
Escalate: print hand-off packet for Claude; do not call Claude.
"""
...
def apply_triage_answer(triage_id: str, answer: str, dry: bool) -> str:
    ...
    # Only allow swapping an existing answer= string for a known id — not patterns/routing.
    # Find block with "id": "..." nearby answer
    pat = re.compile(
    ...
    m = pat.search(text)
    if not m:
        # Python RULES use dict style id/answer
        pat2 = re.compile(
            rf'("id"\s*:\s*"{re.escape(triage_id)}"[\s\S]{{0,400}}?"answer"\s*:\s*")([^"]*)(")',
            re.M,
        )
        m = pat2.search(text)
    if not m:
    ...
    if not dry:
        ...
        try:
            ...
            if problems:
                py.write_text(text, encoding="utf-8")  # rollback
                raise SystemExit(f"triage check() failed: {problems[:3]}")
        except SystemExit:
            raise
        except Exception as e:
            # If check cannot run, still keep the swap but note it
