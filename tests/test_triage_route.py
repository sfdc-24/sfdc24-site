...
T.setCrew(["foundry", "claude", "codex", "gemini", "grok"]);
...
const foundryHit = T.route("azure foundry deployment scoring benchmark");
if (foundryHit.routeTo !== "foundry" || foundryHit.why !== "keyword") {
  console.error("FAIL foundry", JSON.stringify(foundryHit));
  process.exit(1);
}
...
if (a.routeTo !== "foundry") {
  console.error("FAIL default lead not foundry on first RR", JSON.stringify({ a, b }));
  process.exit(1);
}
...
console.log("ok", apex.routeTo, codeAsk.routeTo, foundryHit.routeTo, a.routeTo, b.routeTo);
"""
        proc = subprocess.run(
    ...
    def test_crew_default_leads_with_foundry(self) -> None:
        src = TRIAGE_PY.read_text(encoding="utf-8")
        self.assertIn(
            'CREW_DEFAULT = ["foundry", "claude", "codex", "gemini", "grok"]',
            src,
            "ALL-HANDS-001 appointed foundry as default lead",
        )
