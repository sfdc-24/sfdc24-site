...
test('Method explains inference across build and interaction; homepage shows it', () => {
  ...
  assert.match(method, /data\/inference\//, 'Inference dropped the mine catalog');
  assert.match(method, /aggregates only/, 'Inference dropped hybrid-store aggregates-only');
  ...
  assert.match(doctrine, /Hybrid store/, 'site-doctrine.md dropped hybrid store');
  const catalog = JSON.parse(fs.readFileSync(path.join(REPO, 'data/inference/catalog.json'), 'utf8'));
  assert.equal(catalog.cookies, false, 'inference catalog allows cookies');
  assert.ok(Array.isArray(catalog.datasets) && catalog.datasets.some((d) => d.id === 'eta'), 'catalog missing eta');
  assert.ok(catalog.datasets.some((d) => d.id === 'ab'), 'catalog missing ab');
  assert.ok(fs.existsSync(path.join(REPO, 'data/inference/eta.jsonl')), 'eta.jsonl seed is missing');
  assert.ok(fs.existsSync(path.join(REPO, 'tools/inference_mine.py')), 'tools/inference_mine.py is missing');
  assert.ok(fs.existsSync(path.join(REPO, 'tools/inference_export.py')), 'tools/inference_export.py is missing');
