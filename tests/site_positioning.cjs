...
test('Release rail script is present and homepage boot loads it', () => {
  ...
  assert.match(opsPage, /id="release-clock"/, 'Ops page has no slot for the Release clock');
  assert.match(opsPage, /\/assets\/next-deploy\.js/, 'Ops page does not load the Release clock');
  ...
  assert.doesNotMatch(note, /Readable header and current date/, 'Release sentence is still the stale header/date line');
  assert.doesNotMatch(note, /Cobalt theme site-wide/, 'Release sentence is still the stale theme line');
  ...
  assert.match(src, /<b>Release<\/b>/, 'Release rail lost its label');
  assert.match(src, /id="ndRem"/, 'Release rail must show remaining time');
  assert.match(src, /function watchSvg\(/, 'Release rail must draw the stopwatch');
  assert.match(src, /--:--/, 'Release rail must keep an unset countdown visible');
  assert.match(src, /__SFDC24_NEXT_DEPLOY/, 'Release rail must honor an explicit next ISO');
  assert.match(src, /data-next-deploy/, 'Release rail must honor data-next-deploy');
  assert.match(src, /next-release\.json/, 'Release rail must read the committed next-release config');
  assert.match(src, /__SFDC24_NEXT_NOTE/, 'Release rail must honor an explicit next note');
  assert.doesNotMatch(src, /sendBeacon/, 'Release rail must not POST at a static host');
  ...
  const nextRelease = JSON.parse(fs.readFileSync(path.join(REPO, 'data/next-release.json'), 'utf8'));
  assert.equal(typeof nextRelease.note, 'string', 'data/next-release.json needs a note');
...
test('Cobalt stays the default with a readable dark-header accent', () => {
  ...
  assert.match(rail, /<b>Release<\/b>/, 'Release rail lost its label');
  assert.match(rail, /function watchSvg\(/, 'Release rail must draw the stopwatch');
  assert.match(rail, /#0A66C2/, 'Release stopwatch dropped the Cobalt accent');
